...
function getPages(pdf, first, last){
    const pages = range(first, last+1).map(number => pdf.getPage(number));
    ...
    watch:{
        url:{
            handler(url){
                ...
                .catch(err => {
                    console.log('[PDFData] Failed to retrieve PDF: ' + err);
                });
            },
    ...
    methods:{
        fetchPages(currentPage = 0){
            ...
            getPages(this.pdf, startPage, endPage).then(pages => {
                ...
                console.log('[PDFData] Failed to fetch pages: ' + err);
            })
        }
    },
